...
public interface InternshipOfferDAO extends JpaRepository<InternshipOffer, Long> {
    ...
    @Query(
            ...
            "AND (:program IS NULL OR LOWER(io.program) LIKE LOWER(CONCAT('%', :program, '%'))) " +
            "AND (:location IS NULL OR LOWER(io.address) LIKE LOWER(CONCAT('%', :location, '%'))) " +
            "AND (:jobTitle IS NULL OR LOWER(io.jobTitle) LIKE LOWER(CONCAT('%', :jobTitle, '%'))) " +
            "AND (:company IS NULL OR LOWER(io.employer.enterprise) LIKE LOWER(CONCAT('%', :company, '%'))) " +
