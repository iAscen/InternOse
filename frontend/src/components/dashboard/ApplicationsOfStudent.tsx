...
export default function ApplicationsOfStudent({applications}: ApplicationsOfStudentProps) {
    ...
    const [applicationToConfirm, setApplicationToConfirm] = useState<StudentApplication>()
    const [confirmationType, setConfirmationType] = useState<'REJECT' | 'ACCEPT'>('REJECT')
    ...
    return (
        <>
            ...
            <div className="divide-y divide-gray-200 shadow-lg rounded-lg ms-6 me-6">
                ...
                >
                    ...
                    <div className="flex gap-1 mt-4 ms-auto text-sm">
                        <button onClick={() => {setApplicationToConfirm(application); setConfirmationType('ACCEPT')}} className="rounded-sm px-2 bg-green-500 text-white hover:bg-green-600 hover:cursor-pointer">
                            Accepter
                        </button>
                        <button onClick={() => {setApplicationToConfirm(application); setConfirmationType('REJECT')}} className="rounded-sm px-2 bg-red-500 text-white hover:bg-red-600 hover:cursor-pointer">
                            Refuser
                        </button>
                    </div>}
                    </div>
            ...
            {applicationToConfirm && <ApplicationConfirmationModal 
                    application={applicationToConfirm} 
                    mode={confirmationType}
                    onClose={() => setApplicationToConfirm(undefined)}></ApplicationConfirmationModal>}
        </>
      );
}
